...
- Sub-agent tools (`agent`/`check_agent_result`/`list_agent_tasks`) are likewise `enabled=false` in the table; they are appended by the chat runtime via `chat::sub_agent::append_tool_definitions` and gated by **agent mode**, not a settings flag — exposed in Act + Orchestrate (`supports_tools && !plan_mode`), filtered out in Plan. The former `chat_tools.sub_agents` settings toggle was removed.
