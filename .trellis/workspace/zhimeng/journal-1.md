...
## Session 10: Chat UI: mode pill + sub-agent progress/timeout + stability

**Date**: 2026-06-13
**Task**: Chat UI: mode pill + sub-agent progress/timeout + stability
**Branch**: `chat-stability-resilience`

### Summary

三块迭代提交：①composer 加可见 agent-mode 胶囊切换器(Act/Plan/Orchestrate)②P3 sub-agent 嵌套进度按 tool_call_id 去重+按工具名聚合计数(消除生命周期刷屏)、agent 工具超时从 120s 提到 660s(对齐 sub-agent 300s×2 预算)、超时不重试③Chat UI 稳定性根治：白屏(showStreamSnapshotIfCurrent 改 requestAnimationFrame 合帧+done 立即 flush+切换/卸载取消挂起帧)、闪烁(finish 先填持久化再清预览消除空帧)、ghost 会话(终止全路径清 in-flight + 加载失败/删除时 dropConversationLocally 本地剔除)。cargo 405 + 前端 72 测试全绿。
...
| Hash | Message |
|------|---------|
| `47b58bfb` | (see git log) |
| `b0e21074` | (see git log) |
| `21297626` | (see git log) |
